...
- **Multi-tenancy**: tenant derives from the trusted gateway auth-context (`X-Auth-Tenant`, RFC-03 via `io.labs64.checkout.web.authcontext`); `RequestTenantProvider` supplies it (dev fallback: `labs64.tenant.default`).
